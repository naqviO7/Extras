...
def baner():
    #creating baner of tool
    print(pyfiglet.figlet_format("YFi"))
...
def get_windows_saved_ssids():
    """ Returns a list of saved SSIDs in a Windows machine using netsh command 
            and will get all saved network profiles from PC Memory
    """
...
def print_profiles(verbose=1):
    #getting current os 
        # in which os this code is used.
    current_os=platform.system()

    if current_os == "Windows":
        time.sleep(2)
        print('[+] Displaying All Wifi Networks.')
        show_available_wifi()

        time.sleep(2)
        print('[+] Doumping Password of Wifi')
        ...
        raise NotImplemented("[!] Works Only in Windows and Linux.")
