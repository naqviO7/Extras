...
connector = sqlite3.connect('Quiz_bank.db')
cursor = connector.cursor()

connector.execute(
"CREATE TABLE IF NOT EXISTS QUIZMODULE (MODULEID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, MODULENAME TEXT, DETAIL TEXT)")

# Creating the functions
def reset_fields():
   global name_strvar, detial_strvar

   for i in ['name_strvar', 'detail_strvar']:
       exec(f"{i}.set('')")
...
def display_records():
   ...
   curr = connector.execute('SELECT * FROM QUIZMODULE')
   data = curr.fetchall()  
...
def add_record():
   global name_strvar, deail_strvar

   name = name_strvar.get()
   detail = detail_strvar.get()
   ...
   if not name or not detail:
       ...
       #try:
        connector.execute(
        'INSERT INTO QUIZMODULE (MODULENAME, DETAIL) VALUES (?,?,?,?,?,?)', (name, detail)
        )
        connector.commit()
...
def remove_record():
   if not tree.selection():
       ...
       connector.execute('DELETE FROM QUIZMODULE WHERE MODULEID=%d' % selection[0])
       connector.commit()
...
def view_record():
   global name_strvar, detail_strvar
   ...
   name_strvar.set(selection[1]); detail_strvar.set(selection[2])


# Initializing the GUI window
main = Tk()
main.title('Quiz Management System')
...
detail_strvar = StringVar()


# Placing the components in the main window
Label(main, text="QUIZ MANAGEMENT SYSTEM", font=headlabelfont, bg='SpringGreen').pack(side=TOP, fill=X)
...
Label(left_frame, text="Detail", font=labelfont, bg=lf_bg).place(relx=0.2, rely=0.31)

Entry(left_frame, width=19, textvariable=name_strvar, font=entryfont).place(x=20, rely=0.1)
Entry(left_frame, width=19, textvariable=detail_strvar, font=entryfont).place(x=20, rely=0.36)
...
Label(right_frame, text='Module Records', font=headlabelfont, bg='DarkGreen', fg='LightCyan').pack(side=TOP, fill=X)

tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                   columns=('Module ID', "Name", "Detail"))
...
tree.heading('Module ID', text='ID', anchor=CENTER)
tree.heading('Name', text='Name', anchor=CENTER)
tree.heading('Detail', text='Module Details', anchor=CENTER)
